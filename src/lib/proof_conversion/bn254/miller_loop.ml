...
(** Run the ate loop body for one iteration.
    ...
    - If bit = -1: f = f * line_add(T, -Q, P) *)
let ate_loop_iteration (f : Fp12.Circuit.t)
    ~(double_line : Lines.G2Line.t) ~(add_line : Lines.G2Line.t option)
    ~(cache : Lines.AffineCache.t) ~(bit : int) : Fp12.Circuit.t =
  (* f = f^2 * line_double *)
...
let run ~(initial_f : Fp12.Circuit.t)
  ...
  for i = 1 to n - 1 do
    f :=
      ate_loop_iteration !f ~double_line:double_lines.(i - 1)
        ~add_line:add_lines.(i - 1) ~cache ~bit:ate.(i)
  done ;
  !f
