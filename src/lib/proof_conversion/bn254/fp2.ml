...
let mul (a : Circuit.t) (b : Circuit.t) : Circuit.t =
  ...
  let c1 =
    Step.exists FF.FpU.typ ~compute:(fun () ->
        let a0 = Step.As_prover.read FF.Field3.typ (FpA.to_field3 a.c0) in
        let a1 = Step.As_prover.read FF.Field3.typ (FpA.to_field3 a.c1) in
        let b0 = Step.As_prover.read FF.Field3.typ (FpA.to_field3 b.c0) in
        let b1 = Step.As_prover.read FF.Field3.typ (FpA.to_field3 b.c1) in
...
let inverse (a : Circuit.t) : Circuit.t =
  ...
  (* Nori: t0.assertAlmostReduced().inv().assertAlmostReduced()
