    ...
    let convert ~input_path:_ ~output_path =
      let witnesses =
        Array.init Circuits.num_circuits ~f:(fun _n ->
            ...
            Requests.empty_witness )
      in
      let proofs = Pickles_rules.compile_and_prove_all ~witnesses in
      ...
      let layer1 =
        Array.init 16 ~f:(fun i ->
            ...
            fst
              (Compressor.prove_layer1 ~left_in ~left_out ~right_in ~right_out) )
      in
      ...
      let json_proofs =
        Array.mapi proofs ~f:(fun i proof ->
            `Assoc
              [ ("circuit", `Int i); ("proof", `String (P.to_base64 proof)) ] )
      in
      let output_json =
        `Assoc
          [ ("type", `String "plonk")
