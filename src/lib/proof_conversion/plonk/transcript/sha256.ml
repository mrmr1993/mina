...
let maj (x : Uint32.t) (y : Uint32.t) (z : Uint32.t) : Uint32.t =
  let sum =
    seal Step.Field.(Uint32.to_field x + Uint32.to_field y + Uint32.to_field z)
  in
  let xor_val = Uint32.to_field (Uint32.xor (Uint32.xor x y) z) in
  let maj =
    seal (Step.Field.div (Step.Field.sub sum xor_val) (Step.Field.of_int 2))
  in
  Uint32.of_field maj
...
let sigma_simple (u : Uint32.t) ~(bits : int * int * int) ~(first_shifted : bool)
    : Uint32.t =
...
let sigma (u : Uint32.t) ~(bits : int * int * int) ~(first_shifted : bool) :
    Uint32.t =
  if Uint32.is_constant u then sigma_simple u ~bits ~first_shifted
  else
    let r0, r1, r2 = bits in
    (* TODO assert bits are sorted *)
    ...
    Step.Field.Assert.equal
      (Step.Field.add x0 (Step.Field.mul x123 (shift d0)))
      x ;
...
let compress (h : Uint32.t array) (w : Uint32.t array) : Uint32.t array =
  ...
  for t = 0 to 63 do
    ...
    let unreduced_t1 =
      seal
        Step.Field.(
          Uint32.to_field !hh + Uint32.to_field s1 + Uint32.to_field ch_val
    ...
    let unreduced_t2 =
      Step.Field.(Uint32.to_field s0 + Uint32.to_field maj_val)
    in
    hh := !g ;
    g := !f ;
    f := !e ;
    ...
    e :=
      snd
        (Uint32.div_mod_32
           Step.Field.(Uint32.to_field !d + unreduced_t1)
           ~n_bits:48 ) ;
    d := !c ;
    c := !b ;
    b := !a ;
    ...
    a :=
      snd
        (Uint32.div_mod_32 Step.Field.(unreduced_t2 + unreduced_t1) ~n_bits:48)
  done ;
...
let message_schedule (m : Uint32.t array) : Uint32.t array =
  ...
  for t = 16 to 63 do
    ...
    let unreduced =
      Step.Field.(
        Uint32.to_field d1 + Uint32.to_field w7 + Uint32.to_field d0
        + Uint32.to_field w16)
    in
    (* mod 32bit the unreduced field element *)
...
let initial_state () : Uint32.t array = Array.map h_init ~f:Uint32.of_int
