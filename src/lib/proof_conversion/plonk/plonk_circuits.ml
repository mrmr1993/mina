...
let build_circuit_body ~(circuit_index : int) : circuit_body =
      ...
      fun input_hash ->
       ...
       let lcm_x, lcm_y =
         Piop.compute_commitment_linearized_polynomial_split_1
           ...
           ~vk:plonk_vk ~beta:acc.fs.beta ~gamma:acc.fs.gamma
           ~alpha:acc.fs.alpha
       in
       acc.state.lcm_x <- lcm_x ;
       acc.state.lcm_y <- lcm_y ;
      ...
      fun input_hash ->
       ...
       let lcm_x, lcm_y =
         Piop.compute_commitment_linearized_polynomial_split_2
           ~lcm_x:acc.state.lcm_x ~lcm_y:acc.state.lcm_y ~proof:acc.proof
           ~vk:plonk_vk ~beta:acc.fs.beta ~gamma:acc.fs.gamma
           ~alpha:acc.fs.alpha ~zeta:acc.fs.zeta
           ~alpha_2_l0:acc.state.alpha_2_l0 ~hx:acc.state.hx ~hy:acc.state.hy
       in
       acc.state.lcm_x <- lcm_x ;
       acc.state.lcm_y <- lcm_y ;
       Plonk_accumulator.hash_packed acc
  ...
  | 10 ->
      ...
      fun input_hash ->
       ...
       Step.assert_ (Equal (in_digest, input_hash)) ;
       let cm_x, cm_y =
         Piop.fold_state_2 ~vk:plonk_vk ~cm_x:acc.state.cm_x
           ~cm_y:acc.state.cm_y ~gamma_kzg:acc.fs.gamma_kzg
       in
       let kzg_random =
      ...
      fun input_hash ->
       ...
       let kzg_cm_x, kzg_cm_y, neg_fq_x, neg_fq_y =
         Piop.prepare_pairing_0 ~vk:plonk_vk ~proof:acc.proof
           ~random:acc.state.kzg_random ~cm_x:acc.state.cm_x
           ~cm_y:acc.state.cm_y ~cm_opening:acc.state.cm_opening
       in
  ...
  | 13 | 14 | 15 | 16 ->
      ...
      let all_g2 =
        Plonk_lines.load_lines_from_json (data_dir ^ "/g2_lines.json")
      in
      let all_tau =
      ...
      fun input_hash ->
        ...
        let a_cache =
          Lines.AffineCache.make
            { G1.Circuit.x = kzg.proof.a_x; y = kzg.proof.a_y }
        in
        let b_cache =
          Lines.AffineCache.make
        ...
        kzg.state.lines_hashes_digest <-
          Accumulator_hash.poseidon_hash lines_hashes ;
  ...
  | 17 | 18 | 19 | 20 | 21 | 22 ->
      ...
      fun input_hash ->
        ...
        let full_arr =
          Array.concat [ lhs_hashes; opening_hashes; rhs_hashes ]
        in
        ...
        let field_equals x c =
          ...
          let z =
            Step.exists Step.Field.typ ~compute:(fun () ->
                ...
                if Step.Field.Constant.(equal dv zero) then
                  Step.Field.Constant.zero
