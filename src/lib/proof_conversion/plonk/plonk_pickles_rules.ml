...
let make_rule ~(n : int) : _ Pickles.Inductive_rule.Promise.t =
  ...
  { identifier = Printf.sprintf "plonk-zkp%d" n
  ; prevs = []
  ; main =
...
let compile_and_prove_one ~(n : int) ~(input_hash : Step.Field.Constant.t) :
  ...
  Printf.printf "  [plonk-zkp%d] compiling... %!" n ;
  ...
  let _tag, _cache, (module Proof), provers =
    Pickles.compile_promise
      ...
      ~name:(Printf.sprintf "plonk-zkp%d" n)
...
let compile_and_prove_all () :
  ...
  Printf.printf "Compiling and proving %d PLONK circuits (chained)...\n%!"
    Plonk_circuits.num_circuits ;
  let current_hash = ref Step.Field.Constant.zero in
  let proofs = Array.init Plonk_circuits.num_circuits (fun n ->
    ...
    proof )
  in
  Printf.printf "Final PLONK chain hash: %s\n%!"
